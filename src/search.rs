...
pub(crate) fn card_matches(query: &str, backend: &Backend, card: &Card) -> Result<bool, SearchError> {
    println!("Begin query: '{query}'");
    let (node, _) = parse_query(query)?;
    let card_pool: HashSet<SearchPrinting> = card.printings.iter().map(|printing| SearchPrinting{printing, card}).collect();
        ...
        QueryNode::NumFilter(num_filter) => {
            let results = match num_filter.key {
                ...
                NumericKey::NRDB => card_pool.iter()
                    .filter(|x| x.printing.code.parse().is_ok_and(|v: i32| num_filter.comparator.as_operator(v, num_filter.value)))
                    .copied()
                    .collect(),
